    ...
    companion object {
            ...
            return when (kind) {
                AuthMethod.APPROLE.id -> {
                    AppRoleAuthServer(
                        ...
                        endpoint = (map[VaultConstants.FeatureSettings.ENDPOINT] ?: VaultConstants.FeatureSettings.DEFAULT_ENDPOINT_PATH).removePrefix("/"),
                        roleId = map[VaultConstants.FeatureSettings.ROLE_ID] ?: "",
                        secretId = map[VaultConstants.FeatureSettings.SECRET_ID] ?: ""
                    )
                }
                AuthMethod.AWS_IAM.id -> AwsIam
                AuthMethod.LDAP.id -> LdapServer(requireNotNull(map[VaultConstants.FeatureSettings.USERNAME]), requireNotNull(map[VaultConstants.FeatureSettings.PASSWORD]))
        ...
        fun fromSharedParameters(parameters: Map<String, String>, namespace: String): Auth {
            ...
            return when (kind) {
                ...
                AuthMethod.AWS_IAM.id -> AwsIam
                AuthMethod.LDAP.id -> LdapAgent(requireNotNull(parameters.getOrDefault(getVaultParameterName(namespace, VaultConstants.WRAPPED_TOKEN_PROPERTY_SUFFIX), "")))
